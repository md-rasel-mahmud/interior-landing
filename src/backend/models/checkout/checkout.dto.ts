import { PAYMENT_PROVIDER } from "@/enums/PaymentMethods.enum";
...
const CHECKOUT_DEFAULT_VALUES = {
  ...
  address: "",
  transactionId: "",
  paymentMethod: PAYMENT_PROVIDER.COD,
};
...
const checkoutValidationBackend = z
  .object({
    ...
    paymentMethod: z
      .string()
      .optional()
      .default(PAYMENT_PROVIDER.COD)
      .refine(
        (val) =>
          Object.values(PAYMENT_PROVIDER).includes(val as PAYMENT_PROVIDER),
        {
...
const checkoutValidation = z
  .object({
    ...
    address: z
      ...
      .min(5, "Address is required")
      .max(200, "Address must be less than 200 characters"),
    paymentMethod: z.nativeEnum(PAYMENT_PROVIDER, {
  ...
  .superRefine((data, ctx) => {
    const manualPaymentProviders = [
      ...
      PAYMENT_PROVIDER.ROCKET,
    ];

    if (manualPaymentProviders.includes(data.paymentMethod)) {
